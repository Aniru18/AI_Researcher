...
# config = {"configurable": {"thread_id": 111111111}}
...
def should_continue(state: State) -> Literal["tools", END]:
    """
    Continue to tools if:
    - LLM explicitly calls a tool
    """
...
checkpointer = MemorySaver()
config = {"configurable": {"thread_id": 1111111111}}

graph = workflow.compile(checkpointer=checkpointer)


# ===============================
# Step 6: System Prompt (IMPORTANT UPDATE)
# ===============================
INITIAL_PROMPT = """
...
Finally, I'll ask you to go ahead and write the paper. Make sure that you
include mathematical equations in the paper. Once it's complete, you should
render it as a LaTeX PDF. Make sure that TEX file is correct and there is no error in it so that PDF is easily exported. 
When you give papers references, always attatch the pdf links to the paper
IMPORTANT:
- If LaTeX or PDF generation fails, analyze the error carefully.
